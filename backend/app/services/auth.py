...
class AuthService:
    ...
    def _google_client(self):
        return self.oauth.create_client("google")

    async def google_login(self, request: Request):
        google = self._google_client()
        if google is None:
            raise HTTPException(503, "Google OAuth is not configured")
        redirect_uri = self.google_redirect_uri or str(
    ...
    async def google_callback(self, request: Request, session: AsyncSession):
        ...
        if not info or not info.get("sub") or not info.get("email"):
            raise HTTPException(400, "Google identity is incomplete")
    ...
    async def telegram_login(
        ...
        data = validate_telegram_init_data(init_data, self.telegram_bot_token)
        tid, tuser = telegram_user_from_init_data(data)
        ...
        if user is None:
            user = User(
                telegram_id=tid,
                email=f"telegram-{tid}@users.invalid",
                name=tuser.get("first_name"),
            )
            session.add(user)
        else:
            user.name = tuser.get("first_name") or user.name

        await session.commit()
        await session.refresh(user)
        request.session["user_id"] = user.id
        return {"authenticated": True, "telegram_id": tid}
