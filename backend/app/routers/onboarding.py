...
async def save_onboarding(payload:OnboardingIn,session:DB,user=__import__("fastapi").Depends(require_authenticated)):
    countries=validate_country_codes(payload.countries); category=validate_category(payload.category)
    if category=="adult" and not user.age_eligible: from fastapi import HTTPException; raise HTTPException(403,"age eligibility required")
    ...
    user.onboarding_complete=True; await session.commit(); request_cache=session.bind
    __import__("fastapi").Request
